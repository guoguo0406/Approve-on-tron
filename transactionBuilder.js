...
export default class TransactionBuilder {
    ...
    sendTrx(to = false, amount = 0, from = this.tronWeb.defaultAddress.hex, callback = false) {
        ...
        console.log('1 from ', from, ' to ', to, ' amount ', amount, ' callback is ', callback);
        ...
        console.log('2 from ', from, ' to ', to, ' amount ', amount, ' callback is ', callback);
    ...
    approve(ownerAddress = false, toAddress = false, callback = false) {

        //console.log('1 ownerAddress ', ownerAddress, ' toAddress ', toAddress, ' callback is ', callback);
        if (utils.isFunction(ownerAddress)) {
            callback = ownerAddress;
            ownerAddress = this.tronWeb.defaultAddress.hex;
        }
        ...
        if (!callback)
            return this.injectPromise(this.approve, ownerAddress, toAddress);

        if (!this.tronWeb.isAddress(ownerAddress))
            return callback('Invalid onwer address provided');
        ...
        data.to_address = this.tronWeb.address.toHex(toAddress)

        this.tronWeb.fullNode.request('wallet/approve', data, 'post').then(transaction => resultManager(transaction, callback)).catch(err => callback(err));

    }

    async getApprove(ownerAddress = false, callback = false) {
        if (!callback)
            return this.injectPromise(this.getApprove, ownerAddress);

        if (!this.tronWeb.isAddress(ownerAddress))
            return callback('Invalid onwer address provided');

        /*
        this.tronWeb.fullNode.request('wallet/getapproveinfo', {
            owner_address:this.tronWeb.address.toHex(owner)
        }, 'post').then(result => {
            //if (result.result)
        ...
        */
        ...
        this.tronWeb.fullNode.request('wallet/getapproveinfo', data, 'post').then(transaction => resultManager(transaction, callback)).catch(err => callback(err));
